...
if __name__ == "__main__":
    # 创建多用户对话实例
    chatbot = MultiUserChatbot(host='http://10.2.28.149:11434', model='qwen2.5-20250402:latest')

    # 启动与用户1的对话并设置自定义指令
    user_id = "user1"
    custom_prompt = "每次答案的结尾都必须加一个'喵'字"
    chatbot.set_custom_prompt(user_id, custom_prompt)  # 设置用户1的自定义指令

    chatbot.start_chat(user_id, prompt=custom_prompt)  # 启动对话并传递自定义指令
