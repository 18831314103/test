...
# 第一次加载首条数据
app.clientside_callback(
    ClientsideFunction(namespace='clientside', function_name='render_goChart'),
    Output('ABN-real-module-gojs', 'children', ),
    [
        Input('ABN-real-module-store-clicktable', 'data'),
        Input('ABN-real-module-store-clicktable-dataDesc', 'data')
    ]
...
@app.callback(
    Output('ABNReal-recent-record-card', 'children'),
    Input('ABN-real-module-store-clicktable-data', 'data'),
...
@app.callback(
    [
        Output('ABN-real-module-store', 'data'),
        Output('statistic-info-text', 'htmlString'),
        Output('ABN-real-module-table', 'data'),
    ],
    Input('ABN-temp-interval', 'n_intervals'),
    State('ABNMode-Tabs', 'activeKey'),
    prevent_initial_call=True
...
def store_real_data(n_intervals, activeKey):
    ...
    for filter_condition_item in filter_condition:
        ...
        filter_condition_list.add(filter_condition_item_json["key"])
...
@app.callback(
    [
        ...
        Output('ABN-real-module-store-clicktable', 'data', allow_duplicate=True),
        Output('ABN-real-module-store-clicktable-dataDesc', 'data', allow_duplicate=True),
...
def initABNRealModule(activeKey, pathname):
    if activeKey in [None, '故障模式实时报警'] and pathname == '/ABNModule/ABNRealGxc':
        # query_param = dict(time_delta='36000000')
        # res = AlarmRecordApi.list_alarm_record_in_duration(query_param)
        # if res["code"] == 200:
        #     dcc.Store(id="warn-record-from-mysql-store", data=res["data"])
        dataYCRedis = redis_pool.mget(key='anomaly:alarm_info:*')
        dataEndRedis = redis_pool.mget(key='eng:alarm_info:*')
        sorted_list = simple_format_and_sort(dataYCRedis, dataEndRedis)
        ...
        return [dcc.Interval(id='ABN-temp-interval', interval=REAL_INTERVAL), json_data_redis,
...
@app.callback(
    [
        Output('ABN-real-module-store-clicktable', 'data'),
        Output('ABN-real-module-store-clicktable-dataDesc', 'data'),
        Output('fullscreen-gojs-title', 'children')
    ],
    [
        Input('ABN-real-module-table', 'nClicksCell'),
        Input('ABN-real-module-table', 'nDoubleClicksCell'),
    ],
    [
        State('ABN-real-module-table', 'recentlyCellClickRecord'),
    ],
)
...
@app.callback(
    Output('ABN-real-module-store-clicktable-data', 'data'),
    Input('ABN-real-module-store-clicktable', 'data'),
)
...
@app.callback(
    [
        Output('ABN-real-module-table-warn-rule', 'data', allow_duplicate=True),
        Output('ABN-warn-manager-transfer', 'targetKeys', allow_duplicate=True),
        Output('ABN-real-module-table', 'data', allow_duplicate=True),
        Output('ABN-warn-manager-transfer', 'dataSource', allow_duplicate=True),
    ],
    Input('ABN-warn-manager-transfer-button', 'nClicks'),
    [
        State('ABN-warn-manager-transfer', 'targetKeys'),
        State('ABN-real-module-table', 'data'),
    ],
    prevent_initial_call=True
)
...
@app.callback(
    ...
    Input('ABN-real-module-table-warn-rule', 'nClicksButton'),
    [
        State('ABN-real-module-table-warn-rule', 'recentlyButtonClickedRow'),
...
@app.callback(
    output=(dict(
        ...
        modal_title=Output('menu-modal', 'title', allow_duplicate=True),
        modal_children=Output('menu-modal', 'children', allow_duplicate=True),
        modal_width=Output('menu-modal', 'width', allow_duplicate=True),
    )),
    inputs=(dict(
        n_Clicks=Input('ABN-real-module-warn-manager', 'nClicks'),
    )),
    state=(dict(
        data=State('ABN-real-module-table', 'data'),
    )),
    prevent_initial_call=True,
)
def show_modal(
            ...
            fac.AntdRow(
                [fac.AntdCol(
                    [fac.AntdTransfer(
                        ...
                        fac.AntdButton(
                            "确认",
                            id="ABN-warn-manager-transfer-button",
                    ...
                    fac.AntdCol(
                        fac.AntdTable(
                            id="ABN-real-module-table-warn-rule",
                            columns=[
...
@app.callback(
    output=(dict(
        ...
        modal_children=Output('menu-modal', 'children', allow_duplicate=True),
        modal_width=Output('menu-modal', 'width', allow_duplicate=True),
        render_footer=Output('menu-modal', 'renderFooter', allow_duplicate=True),
        graph_data=Output('ABN-Check-YC-Modal-store', 'data', allow_duplicate=True),
        table_data=Output('ABN-real-module-table', 'data', allow_duplicate=True),
    )),
    inputs=(dict(
        n_Clicks=Input('ABN-real-module-table', 'nClicksButton'),
    )),
    state=(dict(
        click_content=State('ABN-real-module-table', 'clickedContent'),
        click_row=State('ABN-real-module-table', 'recentlyButtonClickedRow'),
        active_key=State('ABNMode-Tabs', 'activeKey'),
        table_data=State('ABN-real-module-table', 'data'),
    )),
    prevent_initial_call=True,
)
def show_modal(
                    ...
                    [
                        fac.AntdCol(
                            [
                                ...
                                html.Div(
                                    id="ABN-Check-YC-Modal-Chart-Container",
                                    style={
                                        'width': '100%',
                                        "height": 400,
                                        "background": "#141414",
...
@app.callback(
    ...
    [State('menu-modal', 'title'),
     State('ABN-real-module-table', 'recentlyButtonClickedRow'),
     State('ABN-real-module-table', 'data'),
     # State('ABN-warn-manager-transfer', 'targetKeys')
     ],
    prevent_initial_call=True,
...
app.clientside_callback(
    ClientsideFunction(namespace='clientside', function_name='render_modal_chart'),
    Output('ABN-real-module-store-no-output', 'data', allow_duplicate=True),
    Input('ABN-Check-YC-Modal-store', 'data'),
    prevent_initial_call=True,
)


@app.callback(
    [Output('fullscreen-gojs', 'isFullscreen')],
    Input('fullscreen-gojs-button', 'nClicks'),
    State('fullscreen-gojs', 'isFullscreen'),
    prevent_initial_call=True
)
