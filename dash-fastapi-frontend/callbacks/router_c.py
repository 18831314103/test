...
from utils.componentCleaner_util import ComponentCleaner
    ...
    if token_result and session_token and token_result == session_token:
        ...
        if pathname in valid_pathname_list:
            if url_trigger == 'load':
                ...
                # result = views.layout.render(user_menu_info)
                # print("序列化预检：", safe_serialize(result))
                layout_raw = views.layout.render(user_menu_info)
                layout_clean = ComponentCleaner.clean(layout_raw)
                # print("类型：", type(layout_clean))
                # print("layout_clean.children 类型：", type(getattr(layout_clean, "children", None)))
                # print("layout_clean.children 长度：", len(getattr(layout_clean, "children", []) or []))
                # print(layout_clean.to_plotly_json())
                # print(repr(layout_clean))
                return dict(
                    app_mount=[layout_clean],
